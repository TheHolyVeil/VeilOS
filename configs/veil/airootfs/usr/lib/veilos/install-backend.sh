...
run_pacstrap() {
  ...
  local packages=(
    base linux linux-firmware grub efibootmgr limine
...
install_bootloader() {
  ...
  # Write the inner chroot script to a temp file and pass vars as arguments
  # (avoids plaintext vars appearing in the outer heredoc and in ps output)
  local chroot_script
  chroot_script=$(mktemp /tmp/veilos-bootloader.XXXXXX.sh)
...
CHROOTEOF
  ...
  arch-chroot /mnt bash "$chroot_script" "$BOOTLOADER" "$BOOT_TYPE" "$DISK"
  rm -f "$chroot_script"
}
...
setup_users() {
  ...
  # Pass passwords via stdin/env rather than positional args to avoid ps exposure
  local chroot_script
  chroot_script=$(mktemp /tmp/veilos-users.XXXXXX.sh)
  chmod 700 "$chroot_script"

  cat >"$chroot_script" <<'CHROOTEOF'
#!/usr/bin/env bash
set -e
# Credentials arrive via environment variables set by the caller
...
setup_desktop() {
  ...
  plasma) arch-chroot /mnt pacman -S --noconfirm plasma ;;
